...
server.tool('kinetic_edit','Atomic persisted edit. Supply the revision you inspected and a retry-safe request ID. At most three parts; metres and DEGREES. Gravity, spawn and target are locked.',{expectedRevision:z.number().int().nonnegative(),requestId:z.string().min(1).max(100),operations:z.array(operation).min(1).max(20)},wrap(args=>client.edit(args)));
server.tool('kinetic_run','Fresh 120 Hz physics for at most 10 simulated seconds. Read success/status: completion is not success. capture returns three timestamped real replay PNGs with a connected browser; view can focus a part.',{expectedRevision:z.number().int().nonnegative(),capture:z.boolean().default(true),view:view.optional()},wrap(args=>client.run(args)));
